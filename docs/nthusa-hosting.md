...
**Calendar.** Set `DISCORD_CALENDAR_ID` to the dedicated **discord-calendar** events calendar, separate from `DISCORD_OFFICE_CALENDAR_ID`. The default guild is `1514899496797212683` and booking user is `nthusa@gapp.nthu.edu.tw`; both have explicit settings. Restore the association's approved OAuth recovery JSON into `DISCORD_CALENDAR_OAUTH_JSON` (the legacy variable also works). For reauthorization, use the `nthusa-discord-calendar` Desktop client with External/Production consent and run `bun scripts/calendar-authorize.mjs CLIENT_JSON OUTPUT_JSON` on the operator's trusted machine. The script accepts account/project overrides through `DISCORD_CALENDAR_ACCOUNT` and `DISCORD_CALENDAR_PROJECT_ID`. Verify sharing and ownership, preserve private recovery storage, and review `/calendar/privacy` before publishing it.

Set the office calendar to **Auto-accept invitations that do not conflict**. Test an online event without office use, explicit office use with accepted RSVP, a rejected conflict, adding another guest after creation, moving away and removing the office invitation, and confirmed deletion. Also test Cancel and another user's denied confirmation. Every mutation waits for the originating requester; office requests may remain pending while Google propagates the invitation. During cutover from the trial, use the same approved Google calendars and enable only one bot's mutation path. Configuration changes invalidate previous confirmation drafts. See the [Calendar reference](configuration.md#calendar-and-invitation-directory).

**Invitation contacts.** Enable Google Sheets API in `nthusa-discord-drive`, retain the service account's Viewer access to the directory, and set the spreadsheet/tab IDs from the examples. The lookup uses the existing read-only Drive scope and permission checks. Test a known name, an ambiguous match and a denied requester; verify the returned fields contain no phone numbers or notes. Select invitees before confirming the Calendar preview.
