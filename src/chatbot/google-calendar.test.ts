...
test("a later invitation can append a person while retaining existing guests and office use", async () => {
  const f = eventWorkflow();
  const made: any = await f.client.call("create_calendar_event", {
    ...creation,
  ...
  const current: any = await f.client.call("get_calendar_event", {
    eventId: made.event.id,
  });
  const people = current.event.attendees
    .filter((guest: any) => guest.email !== OFFICE_ID)
    .map((guest: any) => guest.email);
  const edited: any = await f.client.call("edit_calendar_event", {
    eventId: current.event.id,
    etag: current.event.etag,
    attendees: [...people, "later@example.com"],
  });
  expect(edited.status).toBe("complete");
  expect(
    f.stored.get(made.event.id).attendees.map((guest: any) => guest.email),
  ).toEqual(["first@example.com", "later@example.com", OFFICE_ID]);
  const update = f.requests.find((entry) => entry.init.method === "PATCH")!;
  expect(update.url.searchParams.get("sendUpdates")).toBe("all");
  expect(edited.officeReservation).toBe("pending");
});
