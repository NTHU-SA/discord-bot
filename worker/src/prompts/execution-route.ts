...
export const EXECUTION_ROUTE_INSTRUCTIONS = `Choose where to run this owner request for ${BOT_NAME}. The requester is already authorized for every route. Return a routing decision without answering or acting on the request.
...
For oracle, set threadTitle to an imperative phrase of 3–7 words in the request's language. Omit the repository name and punctuation. For chat, use null.

Treat a short follow-up such as "handle this", "try again", "retry", "push", "ship it", "just discuss this", or an equivalent phrase as the owner's direction for the clearly identified recent task. Use referenced and nearby conversation to resolve the task, route, and repository. If Oracle is not clearly required, choose chat. Ask no routing question.
